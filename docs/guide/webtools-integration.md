...
AutoR is agent-first: users talk to an agent, and the agent orchestrates local AutoR skills.  
...
- You want the agent to combine:
  - AutoR local retrieval (`/autor:search`, `/autor:show`, etc.)
  - external web lookup from claude-webtools.

## Recommended setup

1. Install and configure `claude-webtools` by following its README.
2. Keep AutoR as the authoritative local knowledge pipeline (ingest/index/enrich).
3. In agent workflows:
   - use AutoR first for reproducible local evidence;
...
- When webtools is unavailable, agent should degrade gracefully to local-only AutoR workflows.

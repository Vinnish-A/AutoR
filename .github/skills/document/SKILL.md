...
直接用 Python API 生成 Word / PowerPoint / Excel 文档，并通过 `autor document inspect` 检查文档结构和布局。

## 核心思路

**生成**：不要用 `autor export docx`（那个只是简单的 Markdown 转换器）。本 skill 直接编写 Python 脚本调用 Office 库 API，类似 draw skill 直接调用 Inkscape API 画图。

**检查**：生成后必须用 `autor document inspect <file>` 检查文档，确认布局、内容、图片尺寸无误，再交付给用户。
...
autor document inspect presentation.pptx

# 检查 DOCX：段落/标题/表格/图片结构 + 样式摘要
autor document inspect report.docx

# 检查 XLSX：Sheet 概览 + 数据预览 + 图表列表
autor document inspect data.xlsx
```
...
if img.exists():
    ...
    # 图片标题
    cap = doc.add_paragraph("图 1：AutoR 数据流水线")
...
slide.placeholders[1].text = "AutoR 自动生成\n2026-03-15"
...
6. **检查文档**：运行 `autor document inspect <file>` 检查生成结果
...
```bash
autor export docx --input file.md --output file.docx
```

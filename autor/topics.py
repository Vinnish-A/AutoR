...
复用 paper_vectors 表中的论文级聚合嵌入向量，通过 BERTopic 对全库论文做主题聚类。
支持全库主题概览、单主题论文列表、层级主题树、主题间关联发现。

用法：
    from autor.topics import build_topics, get_topic_overview, get_topic_papers
...
from autor.papers import best_citation as _best_cite
from autor.papers import read_meta as _read_meta
from autor.vectors import _unpack
...
def _patch_pandas_datetime():
    ...
    if not getattr(pd.to_datetime, "_autor_patched", False):
        ...
        _patched_dt._autor_patched = True
...
def _fit_bertopic(
    ...
    *,
    ...
    from autor.vectors import QwenEmbedder
...
def _save_model(model: BERTopic, path: Path) -> None:
    ...
    custom_path = path / "autor_meta.pkl"

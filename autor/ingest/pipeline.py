...
用法（CLI）：
  ...
  autor pipeline --steps toc,l3
  autor pipeline full --dry-run
"""
...
class InboxCtx:
    ...
    Attributes:
        ...
        meta: 提取后的 :class:`~autor.ingest.metadata.PaperMetadata`。
...
def step_extract(ctx: InboxCtx) -> StepResult:
    ...
    Returns:
        ``StepResult.OK`` 成功, ``StepResult.FAIL`` 失败。
    """
    from autor.ingest.extractor import get_extractor
...
def step_dedup(ctx: InboxCtx) -> StepResult:
    ...
    """
    from autor.ingest.metadata import enrich_metadata
...
def step_ingest(ctx: InboxCtx) -> StepResult:
    ...
    Returns:
        ``StepResult.OK`` 成功, ``StepResult.FAIL`` 失败。
    """
    from autor.ingest.metadata import generate_new_stem, write_metadata_json
    from autor.papers import generate_uuid
...
def _move_to_pending(
    ...
    *,
    ...
    """
    from autor.ingest.metadata import metadata_to_dict
    ...
    if not dir_name and ctx.meta and ctx.meta.title:
        from autor.ingest.metadata import generate_new_stem
...
def _ensure_registry_schema(conn, db_path: Path) -> None:
    ...
    try:
        ...
        _log.warning(
            "Duplicate publication_number values found; creating non-unique index. Run 'autor index' to rebuild."
        )
        conn.execute(
